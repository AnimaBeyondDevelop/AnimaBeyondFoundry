...
export function computeCombatResult(attackData, defenseData) {
  const defenderToken =
    (defenseData.defenderTokenId && (canvas.tokens?.get?.(defenseData.defenderTokenId)
  ...
  const attackerActor = attackData.attackerId ? game.actors?.get?.(attackData.attackerId) : null;
  ...
  const lifePercentRemoved = finalDamage / lifeBeforeAttack * 100;
  ...
  return ABFCombatResultData.builder()
    ...
    .lifePercentRemoved(Math.floor(lifePercentRemoved))
...
function getFinalBaseDamage(attackData, defenseData) {
  const finalBaseDamage = Math.max(0, (attackData.damage ?? 0) - (defenseData.damageReduction ?? 0));
