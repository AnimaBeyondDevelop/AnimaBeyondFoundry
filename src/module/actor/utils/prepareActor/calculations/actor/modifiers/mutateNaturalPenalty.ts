...
export const mutateNaturalPenalty = (data: ABFActorDataSourceData) => {
  ...
  let naturalPenaltyReduction = Math.min(-armorsNaturalPenalty,Math.max(0,wearArmor-wearArmorRequirement));
  ...
  data.general.modifiers.naturalPenalty.final.value = unreducedNaturalPenalty + naturalPenaltyReduction + data.general.modifiers.naturalPenalty.special.value;
};
