...
export default class ModifyDicePermissionsConfig extends FormApplication {
  ...
  getData() {
    return {
      roles: Object.entries(CONST.USER_ROLES)
        ...
        .map(([key, val]) => ({ key, val, label: game.i18n.localize(`USER.Role${key.capitalize()}`), checked: this.settings[val] })),
    };
  }

  get settings() {
    return game.settings.get("animabf", ABFSettingsKeys.MODIFY_DICE_FORMULAS_PERMISSION) || {};
  }
  ...
  async _updateObject(event, formData) {
    ...
    await game.settings.set("animabf", ABFSettingsKeys.MODIFY_DICE_FORMULAS_PERMISSION, newSettings);
  }
}
