export const resetDefensesCounterHook = (keepAccumulating) => {
  ...
  if (combatantsAcc.length !== 0) {
    for (let combantantAcc of combatantsAcc) {
      combantantAcc.actor.resetDefensesCounter(keepAccumulating)
    }
  };
  ...
  if (combatants.length == 0) {return}
    ...
    combantant.actor.resetDefensesCounter(keepAccumulating)
    }
  }
}