 ...
 */
function buildTS() {
	return gulp.src('src/**/*.ts').pipe(tsConfig()).pipe(gulp.dest('../animabf'));
}
 ...
 */
function buildLess() {
	return gulp.src('src/*.less').pipe(less()).pipe(gulp.dest('../animabf'));
}
	...
	try {
		for (const file of statics) {
			if (fs.existsSync(path.join('src', file))) {
				await fs.copy(path.join('src', file), path.join('../animabf', file));
			}
		}
...
async function clean() {
	...
	try {
		for (const filePath of files) {
			await fs.remove(path.join('../animabf', filePath));
		}
		return Promise.resolve();
	} catch (err) {
		Promise.reject(err);
...
async function linkUserData() {
	...
	try {
		if (
			fs.existsSync(path.resolve('.', '../animabf', 'module.json')) ||
			fs.existsSync(path.resolve('.', 'src', 'module.json'))
		) {
			destDir = 'modules';
		} else if (
			fs.existsSync(path.resolve('.', '../animabf', 'system.json')) ||
